...
marker_vis = markers.copy().astype(np.float32)
marker_vis[marker_vis == -1] = 0
marker_vis = cv2.normalize(marker_vis, None, 0, 255, cv2.NORM_MINMAX)
...
ax[1,0].imshow(marker_vis, cmap="nipy_spectral")
ax[1,0].set_title("Watershed Markers")

ax[1,1].imshow(dist, cmap="jet")
ax[1,1].set_title("Distance Transform")

ax[1,2].imshow(cv2.cvtColor(output, cv2.COLOR_BGR2RGB))
ax[1,2].set_title(f"Count (Total={rbc_count}, Infected={infected_count})")
